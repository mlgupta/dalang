...
- It creates a user ```TerraformUser``` for the ```org``` account. This account has access only to terraform S3 backend, and it has ability to assume role against target account. Credentials for this users are generated by ```iac-boot.yml``` in ```org_user_cred_file``` as defined in all.yml.
